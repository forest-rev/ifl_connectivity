# -*- coding: utf-8 -*-
"""
Created on Mon Aug 13 15:57:19 2018
This script reads in a csv of patch pairs and runs operations on a selected
patch pair.
@author: pj276
"""
...
if not os.path.exists(adfname_temp):
    ...
    area1 = [] # list for polygon area, 1st patch
    tcarea1 = [] # list for canopy area, 2nd patch
    area2 = [] # list for polygon area, 2nd patch
    tcarea2 = [] # # list for canopy area, 2nd patch
    pid1 = [] # patch 1 id
    pid2 = [] # patch 2 id
    ...
    with fiona.open(oint1, "r") as shapefile:
        geoms = [feature["geometry"] for feature in shapefile]
    with rasterio.open(tcvrt) as src:
        out_image, out_transform = mask(src, geoms, crop=True)
        out_meta = src.meta.copy()
    tcsum = np.sum((out_image/100.0)*xint*yint/1000000)
    tcarea1.append(tcsum)
    gdf = gpd.read_file(oint1)
    polysum = gdf['geometry'].map(lambda p: p.area / 10**6).tolist()[0]
    area1.append(polysum)
    
    # Get polygon masked array for patch 2
    with fiona.open(oint2, "r") as shapefile:
        geoms = [feature["geometry"] for feature in shapefile]
    with rasterio.open(tcvrt) as src:
        out_image, out_transform = mask(src, geoms, crop=True)
        out_meta = src.meta.copy()
    tcsum = np.sum((out_image/100.0)*xint*yint/1000000)
    tcarea2.append(tcsum)
    gdf = gpd.read_file(oint2)
    polysum = gdf['geometry'].map(lambda p: p.area / 10**6).tolist()[0]
    area2.append(polysum)
    ...
    # Dump lists to data frame and save
    areadf = pd.DataFrame({'pid1': pid1, 'pid2': pid2,
                            'area1': area1,'tcarea1': tcarea1,
                            'area2': area2, 'tcarea2': tcarea2})
    areadf.to_csv(adfname_temp)
    
    # Write text file when completed.
    with open(odir + ppc4redodir + '/acomp_' + aname + '.txt', 'wb') as myfile:
        myfile.write('Finished this point pair.')
