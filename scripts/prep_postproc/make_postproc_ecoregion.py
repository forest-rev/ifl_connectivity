# -*- coding: utf-8 -*-
"""
Created on Sun Oct  6 22:44:06 2019
Make scripts for making point post processing scripts for ecoregions
@author: pj276
"""
...
runS1 = False # net flux calculations
runS2 = False # direct flux calculations
runS3 = True # annual tree cover loss and flux calculations
runS4 = True # aggregate csvs produced by S1 or S2
...
if runP1 == True:
    ...
    inshp = '/scratch/pj276/ifl_corridors/ecoregions/Ecoregions2017_nori.shp'
    ...
    fluxcsv = 'wwf_lybytc2000_flux_sums.csv'
    ...
    aggcsvdir = 'wwf_lybytc2000_fluxes'
...
for i in ['af','as','sa']:
    for j in ['neg2', 'neg01', 'pos2']:
        for k in ['2000', '2013']:
            ...
            if not os.path.exists(idir + '/rp5k130k90m_' + j + '_' + i + '_' + k + '_ifl/cfinal/flux_wwf'):
                os.mkdir(idir + '/rp5k130k90m_' + j + '_' + i + '_' + k + '_ifl/cfinal/flux_wwf')
            if not os.path.exists(idir + '/rp5k130k90m_' + j + '_' + i + '_' + k + '_ifl/cfinal/dflux_wwf'):
                os.mkdir(idir + '/rp5k130k90m_' + j + '_' + i + '_' + k + '_ifl/cfinal/dflux_wwf')
            if not os.path.exists(idir + '/rp5k130k90m_' + j + '_' + i + '_' + k + '_ifl/cfinal/lybytc2000_wwf'):
                os.mkdir(idir + '/rp5k130k90m_' + j + '_' + i + '_' + k + '_ifl/cfinal/lybytc2000_wwf')

# Get directory names from aggcsvdir above
if runS1 == True and runS4 == True:
    if not os.path.exists(idir + '/wwf_fluxes'):
        os.mkdir(idir + '/wwf_fluxes')

if runS2 == True and runS4 == True:
    if not os.path.exists(idir + '/wwf_dfluxes'):
        os.mkdir(idir + '/wwf_dfluxes')
        
if runS3 == True and runS4 == True:
    if not os.path.exists(idir + '/wwf_lybytc2000_fluxes'):
        os.mkdir(idir + '/wwf_lybytc2000_fluxes')

#%%
if runS1 == True:
    ##---- Make hf2_flux_by_ecoregion_xx.py scripts ----
    repldict = {"scriptname": "hf2_flux_by_ecoregion", "paramoldname": "param_file", "paramnewname": pbase,
    ...
    ##---- Make hf2_flux_by_ecoregion_submit_xx.py scripts ----
    ttwrite = """#!/bin/bash
    #SBATCH --job-name=fbe
    #SBATCH --chdir=/scratch/pj276/ifl_corridors/output
    #SBATCH --output=/scratch/pj276/ifl_corridors/logs/fbe_%A_%a.log
    ...
    srun ~/.conda/envs/hifo/bin/python /home/pj276/projects/ifl_corridors/code/scripts/prep_postproc/hf2_flux_by_ecoregion_{region}_{scenario}.py ${SLURM_ARRAY_TASK_ID}
    """
    ttwrite = inspect.cleandoc(ttwrite)
    # Get number of jobarrays
    # and write into the template and then write to file
    # Input zone shapefile (copied from above)
    #inshp = '/scratch/pj276/undp_connectivity/vectors/WWF_ecoregions.shp'
    inshp = '/scratch/pj276/ifl_corridors/ecoregions/Ecoregions2017_nori.shp'
    ...
    for i in ['af','as','sa']:
        for j in ['neg2', 'neg01', 'pos2']:
            if os.path.exists(inshp):
                ...
                ucode = np.unique(shpfile['ECO_ID'].tolist())
                ...
                with open(mdir + '/hf2_flux_by_ecoregion_submit_' + i + '_' + j + '.sh','w') as myfile:
                    myfile.write(ttwrite.format(**context))
        
    ##---- SUBMIT JOBS ----
    #for i in ['af', 'as', 'sa']:
    #    for j in ['neg2', 'neg01', 'pos2']:
    #        cmd = "sbatch " +   mdir + '/hf2_flux_by_ecoregion_submit_' + i + '_' + j + '.sh'
...
#%%
if runS2 == True:
    ##---- Make hf2_dflux_by_ecoregion_xx.py scripts ----
    repldict = {"scriptname": "hf2_dflux_by_ecoregion", "paramoldname": "param_file", "paramnewname": pbase,
            "syspathold": "/home/pj276/projects/ifl_corridors/code/parameter_files/",
            "syspathnew": "/home/pj276/projects/ifl_corridors/code/scripts/prep_postproc/"}
    for i in ['af','as','sa']:
        for j in ['neg2', 'neg01', 'pos2']:
    ...
    ##---- Make hf2_dflux_by_ecoregion_submit_xx.py scripts ----
    ttwrite = """#!/bin/bash
    #SBATCH --job-name=dfbe
    #SBATCH --chdir=/scratch/pj276/ifl_corridors/output
    #SBATCH --output=/scratch/pj276/ifl_corridors/logs/dfbe_%A_%a.log
    ...
    srun ~/.conda/envs/hifo/bin/python /home/pj276/projects/ifl_corridors/code/scripts/prep_postproc/hf2_dflux_by_ecoregion_{region}_{scenario}.py ${SLURM_ARRAY_TASK_ID}
    """
    ttwrite = inspect.cleandoc(ttwrite)
    # Get number of jobarrays
    # and write into the template and then write to file
    ...
    for i in ['af','as','sa']:
        for j in ['neg2', 'neg01', 'pos2']:
            if os.path.exists(inshp):
                ...
                with open(mdir + '/hf2_dflux_by_ecoregion_submit_' + i + '_' + j + '.sh','w') as myfile:
                    myfile.write(ttwrite.format(**context))
        
#    #---- SUBMIT JOBS ----
#    for i in ['af', 'as', 'sa']:
#        for j in ['neg2', 'neg01', 'pos2']:
#            cmd = "sbatch " +   mdir + '/hf2_dflux_by_ecoregion_submit_' + i + '_' + j + '.sh'
#            print "Submitting Job " + i + " with command: %s" % cmd
#            status, jobnum = commands.getstatusoutput(cmd)
#            if (status == 0):
#                print "Job1 is %s" % jobnum
...
#%%
if runS3 == True:
    ##---- Make hf2_lyby2000_ecoregion_stats_xx.py scripts ----
    repldict = {"scriptname": "hf2_lyby2000_ecoregion_stats", "paramoldname": "param_file", "paramnewname": pbase,
            "syspathold": "/home/pj276/projects/ifl_corridors/code/parameter_files/",
            "syspathnew": "/home/pj276/projects/ifl_corridors/code/scripts/prep_postproc/"}
    for i in ['af','as','sa']:
        for j in ['neg2', 'neg01', 'pos2']:
    ...
    ##---- Make hf2_lyby2000_ecoregion_stats_submit_xx.py scripts ----
    ttwrite = """#!/bin/bash
    ...
    srun ~/.conda/envs/hifo/bin/python /home/pj276/projects/ifl_corridors/code/scripts/prep_postproc/hf2_lyby2000_ecoregion_stats_{region}_{scenario}.py ${SLURM_ARRAY_TASK_ID}
    """
    ttwrite = inspect.cleandoc(ttwrite)
    # Get number of jobarrays
    # and write into the template and then write to file
    ...
    for i in ['af','as','sa']:
        for j in ['neg2', 'neg01', 'pos2']:
            if os.path.exists(inshp):
                ...
                with open(mdir + '/hf2_lyby2000_ecoregion_stats_submit_' + i + '_' + j + '.sh','w') as myfile:
                    myfile.write(ttwrite.format(**context))
        
#    #---- SUBMIT JOBS ----
#    for i in ['af', 'as', 'sa']:
#        for j in ['neg2', 'neg01', 'pos2']:
#            cmd = "sbatch " +   mdir + '/hf2_lyby2000_ecoregion_stats_submit_' + i + '_' + j + '.sh'
#            print "Submitting Job " + i + " with command: %s" % cmd
#            status, jobnum = commands.getstatusoutput(cmd)
#            if (status == 0):
#                print "Job1 is %s" % jobnum
